...
df_filtrado = rotas_final[(rotas_final['CO.Fabrica'] == co_fabrica) & (rotas_final['Incoterm'] == incoterm)].copy()
...
for index, row in df_final.iterrows():
    var_dict[index] = pulp.LpVariable(f'var_{index}', lowBound=0)

# Definição da função objetivo (apenas uma vez)
# Calcula a função objetivo
objective = pulp.lpSum(
    (row["Dist"] * row["Qtd.Transp"]) / row["Vlr.Frete"] * var_dict[index] 
    for index, row in df_final.iterrows() 
    if row["Vlr.Frete"] != 0
)
prob += objective
...
for index, row in df_final.iterrows():
    prob += var_dict[index] <= row["Capacidade"]

# Resolvendo o problema
prob.solve()

# Adicionando os resultados ao DataFrame
df_final["Custo_Remessa"] = 0.0  # Inicializa a coluna com zeros
for index, row in df_final.iterrows():
    if row["Vlr.Frete"] != 0:
